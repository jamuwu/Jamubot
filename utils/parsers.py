...
def parseReplay(filename):
    ...
    data['modstr'] = num_to_mod(mods)
    data['modbit'] = mods
    graphString, offset = parseString(osr, offset)
    data['life_graph'] = graphString
    ...
    # accuracy calculation
    misses  = data['num_miss']
    num300  = data['num_300']
    num100  = data['num_100']
    num50   = data['num_50']
    numGeki = data['num_geki']
    numKatu = data['num_katu']

    # osu!std
    if data['mode'] == 0:
        numNotes = misses + num300 + num100 + num50
        weightedScore = num300 + num100 * 2.0/6.0 + num50 * 1.0/6.0
        data['accuracy'] = weightedScore / numNotes
    ...
    # catch the beat
        ...
        numNotes = num300 + num100 + num50 + misses + numKatu
        weightedScore = num300 + num100 + num50
        data['accuracy'] = float(weightedScore) / numNotes

        if data['accuracy'] == 1.0:
            data['grade'] = 'SS'
        elif data['accuracy'] > .98:
            data['grade'] = 'S'
        elif data['accuracy'] > .94:
            data['grade'] = 'A'
...
def parse_object(line, mode):
    ...
    # mania
        ...
        if (objtype & 1) != 0:
            ...
            return HitObject(x, y, time, True, params[5])
...
def calculate_acc(beatmap, gamemode:int=0):
    if gamemode == 0:
        total_unscale_score = float(beatmap['num_300'])
        total_unscale_score += float(beatmap['num_100'])
        total_unscale_score += float(beatmap['num_50'])
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score *=300
        user_score = float(beatmap['num_300']) * 300.0
        user_score += float(beatmap['num_100']) * 100.0
        user_score += float(beatmap['num_50']) * 50.0
    elif gamemode == 1:
        total_unscale_score = float(beatmap['num_300'])
        total_unscale_score += float(beatmap['num_100'])
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score *= 300
        user_score = float(beatmap['num_300']) * 1.0
        user_score += float(beatmap['num_100']) * 0.5
        user_score *= 300
    elif gamemode == 2:
        total_unscale_score = float(beatmap['num_300'])
        total_unscale_score += float(beatmap['num_100'])
        total_unscale_score += float(beatmap['num_50'])
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score += float(beatmap['num_katu'])
        user_score = float(beatmap['num_300'])
        user_score += float(beatmap['num_100'])
        user_score  += float(beatmap['num_50'])
    elif gamemode == 3:
        total_unscale_score = float(beatmap['num_300'])
        total_unscale_score += float(beatmap['num_geki'])
        total_unscale_score += float(beatmap['num_katu'])
        total_unscale_score += float(beatmap['num_100'])
        total_unscale_score += float(beatmap['num_50'])
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score *=300
        user_score = float(beatmap['num_300']) * 300.0
        user_score += float(beatmap['num_geki']) * 300.0
        user_score += float(beatmap['num_katu']) * 200.0
        user_score += float(beatmap['num_100']) * 100.0
        user_score += float(beatmap['num_50']) * 50.0
    return (float(user_score)/float(total_unscale_score)) * 100.0

def no_choke_acc(beatmap, gamemode:int=0):
    if gamemode == 0:
        total_unscale_score = float(beatmap['num_300'])
        total_unscale_score += float(beatmap['num_100'])
        total_unscale_score += float(beatmap['num_50'])
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score *=300
        user_score = (float(beatmap['num_300']) + float(beatmap['num_miss'])) * 300.0
        user_score += float(beatmap['num_100']) * 100.0
        user_score += float(beatmap['num_50']) * 50.0
    elif gamemode == 1:
        total_unscale_score = float(beatmap['num_300']) 
        ...
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score *= 300
        user_score = (float(beatmap['num_300']) + float(beatmap['num_miss'])) * 1.0
        user_score += float(beatmap['num_100']) * 0.5
        user_score *= 300
    elif gamemode == 2:
        total_unscale_score = float(beatmap['num_300'])
        ...
        total_unscale_score += float(beatmap['num_miss'])
        total_unscale_score += float(beatmap['num_katu'])
        user_score = (float(beatmap['num_300']) + float(beatmap['num_miss']))
        user_score += float(beatmap['num_100'])
        user_score  += float(beatmap['num_50'])
    elif gamemode == 3:
        total_unscale_score = float(beatmap['num_300'])
