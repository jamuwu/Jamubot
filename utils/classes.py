...
class Recent(Json):
  ...
  def as_embed(self):
    ...
    **{pp}** x{self['max_combo']}/{self['bmap'].maxCombo()} {self['accuracy'] * 100:.2f}%
    ...
    {f"{fc} for {self['bmap'].get_pp.getAccFromValues(n300, n100, n50, miss):.2f}% FC" if self['max_combo'] != self['bmap'].maxCombo() else ''}''', colour=0x00FFC0)
  ...
  def as_embed(self):
    ...
    for i, s in enumerate(self):
      ...
      **{s.get_pp:.2f}PP** {s['max_combo']}/{s['bmap'].maxCombo()} {s['accuracy'] * 100:.2f}%
      ...
      {s.parse_stamp()} ago
      {f"{self.get_fc}PP for {self['bmap'].get_pp.getAccFromValues(n300, n100, n50, miss):.2f}% FC" if self['max_combo'] != self['bmap'].maxCombo() else ''}'''
